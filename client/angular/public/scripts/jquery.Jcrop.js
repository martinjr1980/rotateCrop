...
! function($) {
    $.Jcrop = function(obj, opt) {
        ...
        if ("IMG" == obj.tagName) {
            ...
            // Modified
            var new_width = $(window).width() - 400;
            var new_height = ($origimg.height() / $origimg.width()) * (new_width);
            xscale2 = $origimg.width() / new_width;
            yscale2 = $origimg.height() / new_height;
            $img.width(new_width), $img.height(new_height), $origimg.after($img).hide()
            // Modified

        } else $img = $origimg.css(img_css).show(), img_mode = !1, null === options.shade && (options.shade = !0);
        presize($img, options.boxWidth, options.boxHeight);
        var boundx = $img.width(), //Modified
            boundy = $img.height(), //Modified
        ...
        var api = {
            ...
            getScaleFactor: function() {
                return [xscale2, yscale2] //Modified
            },
            getOptions: function() {
                return options
            },
